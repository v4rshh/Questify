...
import type { Feedback, Game, MapLevel, WizardMood } from './quest-map.types';
...
export default function QuizModal({ open, level, game, loading, onClose, onAnswer, onLevelComplete, onMoodChange }: Props) {
  ...
  useEffect(() => {
    setSelected(null); setFeedback(null); setHintOpen(false); setError(''); updateMood('neutral');
    // Reset only when the level or checkpoint changes.
  ...
  async function checkAnswer() {
    ...
    try {
      ...
      setFeedback(result);
      updateMood(result.correct ? 'celebrating' : 'puzzled');
    } catch (err) {
      setError(err instanceof Error ? err.message : 'The spell fizzled. Please try again.');
  ...
  function advance() {
    ...
    else { setFeedback(null); setSelected(null); setHintOpen(false); updateMood('neutral'); }
  }
      ...
      {open && (
        <motion.div className="backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
          <motion.section className="quiz-scroll" initial={{ opacity: 0, y: 28, scale: .97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: .98 }} role="dialog" aria-modal="true" aria-label={level?.title || 'Level quiz'}>
            <header>
              <button className="close" onClick={onClose} aria-label="Close level">×</button>
              <div className="progress"><i style={{ width: game ? `${Math.min(100, ((game.solved_count + (feedback?.correct ? 1 : 0)) / game.total) * 100)}%` : '0%' }} /></div>
              <span>{game ? `${Math.min(game.solved_count + 1, game.total)} / ${game.total}` : '—'}</span>
            </header>

            {loading || !game || !level ? <div className="loading">The Wizard is preparing your trial…</div> : <>
              <button className={`hint-banner ${hintOpen ? 'open' : ''}`} onClick={() => { const next = !hintOpen; setHintOpen(next); updateMood(next ? 'thinking' : 'neutral'); }}>
                <div className={`wizard-face ${mood}`}><span /></div>
                <div><b>{level.isFinal ? "Wise Wizard’s Whisper" : 'Wizard Hint'}</b><p>{hintOpen ? game.question?.hint : 'Tap to reveal a clue from your concept trail.'}</p></div>
                <strong>{hintOpen ? '−' : '+'}</strong>
              </button>

              <div className="quest-heading">
                <span>{level.isFinal ? 'Roleplay boss encounter' : `${game.difficulty} concept trial`}</span>
                <h2>{level.isFinal ? `Deep within the Great Archive of ${level.title}…` : level.title}</h2>
                {level.isFinal && <p>The final guardian blocks the road. Choose the artifact that restores the knowledge rune.</p>}
              </div>

              {game.completed && !feedback ? (
                <div className="already-complete"><span>🏆</span><h3>Trail already mastered</h3><p>You can review this lesson or return to the map.</p><button onClick={() => onLevelComplete(game)}>View milestone</button></div>
              ) : game.question && <>
              ...
              <AnimatePresence>
                {feedback && <motion.div className={`feedback ${feedback.correct ? 'correct' : 'wrong'}`} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
                  <div className="feedback-icon">{feedback.correct ? '✦' : '↻'}</div>
                  <div><b>{feedback.correct ? 'Rune restored!' : 'Not quite—your map just improved.'}</b><p>{feedback.explanation}</p></div>
                  {feedback.correct ? <button onClick={advance}>{feedback.game.completed ? 'Claim milestone' : 'Next question'} →</button> : <button onClick={() => { setFeedback(null); setSelected(null); updateMood('thinking'); }}>Try again</button>}
                </motion.div>}
              </AnimatePresence>
